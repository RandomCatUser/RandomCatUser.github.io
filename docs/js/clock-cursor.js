// clockCursor — vendored from https://github.com/tholman/cursor-effects
// (MIT). Adapted from UMD `export` to a plain browser global so it
// works with simple script tags. Original draws a 90s-style analog
// clock + date trail that follows the cursor on canvas.
window.clockCursor = function clockCursor(options) {
  ...
  const outlineColor = (options && options.outlineColor) || null;
  ...
  function init() {
    ...
    canvas.style.zIndex = (options && options.zIndex) || "9999999999";
    ...
    // fix: original source loops the wrong arrays here (tmpm/tmps are
    // empty at this point); iterate the hand definitions instead
  ...
  function updateParticles() {
    ...
    for (let i = 0; i < tmpd.length; i++) {
      ...
      if (outlineColor) { context.strokeStyle = outlineColor; context.lineWidth = 3; context.strokeText(tmpd[i].value, tmpd[i].x, tmpd[i].y); }
    ...
    for (let i = 0; i < tmpf.length; i++) {
      ...
      if (outlineColor) { context.strokeStyle = outlineColor; context.lineWidth = 3; context.strokeText(tmpf[i].value, tmpf[i].x, tmpf[i].y); }
    ...
    for (let i = 0; i < tmph.length; i++) {
      ...
      if (outlineColor) { context.strokeStyle = outlineColor; context.lineWidth = 3; context.strokeText(tmph[i].value, tmph[i].x, tmph[i].y); }
    ...
    for (let i = 0; i < tmpm.length; i++) {
      ...
      if (outlineColor) { context.strokeStyle = outlineColor; context.lineWidth = 3; context.strokeText(tmpm[i].value, tmpm[i].x, tmpm[i].y); }
    ...
    for (let i = 0; i < tmps.length; i++) {
      ...
      if (outlineColor) { context.strokeStyle = outlineColor; context.lineWidth = 3; context.strokeText(tmps[i].value, tmps[i].x, tmps[i].y); }
